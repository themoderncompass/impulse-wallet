...
export async function onRequestPost({ request, env }) {
  try {
    const { roomCode, player, weekKey, areas } = await request.json().catch(() => ({}));

    if (!roomCode) return json({ error: "roomCode required" }, 400);
    if (!validWeekKey(weekKey || "")) return json({ error: "valid weekKey (YYYY-MM-DD) required" }, 400);
    if (!Array.isArray(areas)) return json({ error: "areas must be an array" }, 400);

    const clean = Array.from(new Set(areas.map(a => String(a).trim()).filter(Boolean)));
